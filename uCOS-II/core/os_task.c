...
/*
*********************************************************************************************************
*                             SUSPEND A TASK IF THE count IS EQUAL TO 0.
*
* Description: This function extend from OSTaskSuspend(). tds 2014.8.25
*
* Arguments  : prio     is the priority of the task to suspend.  If you specify OS_PRIO_SELF, the
*                       calling task will suspend itself and rescheduling will occur.
*
*            : count    is condition for the task to suspend. The task will suspend if count is equal to 0.
*
* Returns    : OS_ERR_NONE               if the requested task is suspended
*              OS_ERR_TASK_SUSPEND_IDLE  if you attempted to suspend the idle task which is not allowed.
*              OS_ERR_PRIO_INVALID       if the priority you specify is higher that the maximum allowed
...
#if OS_TASK_SUSPEND_EN > 0u
INT8U  OSTaskSuspendIf0 (INT8U prio, INT32U *count)
{
    BOOLEAN    self;
    OS_TCB    *ptcb;
    INT8U      y;
...
#endif
    OS_ENTER_CRITICAL();
	if (*count != 0u)	{			                            /* Not allowed to suspend this task    */
        OS_EXIT_CRITICAL();
        return (OS_ERR_TASK_SUSPEND_IDLE);
	}
    if (prio == OS_PRIO_SELF) {                                 /* See if suspend SELF                 */
